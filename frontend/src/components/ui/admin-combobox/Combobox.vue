...
const rawModelValue = defineModel<string | string[] | undefined | null>()
...
const modelValue = computed<string | string[] | undefined>({
  get() {
    return rawModelValue.value === null ? undefined : rawModelValue.value;
  },
  set(value) {
...
const valueLabel = computed(() => {
  if (!props.multiple) {
    return props.items.find((item) => item[props.value ?? 'id'] === modelValue.value)?.[props.label ?? 'value']
  }

  return props.items
    .filter((item) => modelValue.value?.includes(item[props.value ?? 'id']))
    .map((item) => item[props.label ?? 'value'])
    .join(', ')
})
...
const valuesNormalized = computed(() =>
  props.items.map((i) => normalize(i[props.label ?? 'value'])),
)
