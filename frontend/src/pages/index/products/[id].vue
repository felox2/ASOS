<template>
	<Card class="container mx-auto p-4 mt-10" v-if="product">
		<div class="product-detail max-w-4xl mx-auto rounded-lg p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
			<div class="w-full sm:w-auto">
				...
				<Carousel class="relative w-full max-w-xs" @init-api="(val) => emblaThumbnailApi = val">
					<CarouselContent class="flex gap-1 ml-0">
						<CarouselItem v-for="(photo, index) in allPhotos" :key="index" class="pl-0 basis-1/4 cursor-pointer"
      ...
      <div class="relative">
        ...
        <div class="brand-info flex items-center mb-6 absolute bottom-0 left-0 w-full p-4 bg-white">
          <div class="brand-photo-container w-16 h-16 bg-gray-200 rounded-full flex items-center justify-center mr-4">
            <img v-if="product?.brand?.photo" :src="product.brand.photo" alt="Brand photo"
